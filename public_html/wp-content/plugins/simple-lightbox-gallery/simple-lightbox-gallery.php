...
function slgf_SettingsPage() {
	...
	add_submenu_page( 'edit.php?post_type=slgf_slider', __( 'Recommendation', WEBLIZAR_SLGF_TEXT_DOMAIN ), __( 'Recommendation', WEBLIZAR_SLGF_TEXT_DOMAIN ), 'administrator', 'SLGF-Recommendation-Page', 'SLGF_Recommendation_Page' );
}
...
function SLGF_Pro_page_Function() {
	//css
	wp_enqueue_style( 'wrgf-font-awesome', WEBLIZAR_SLGF_PLUGIN_URL . 'css/font-awesome-latest/css/fontawesome-all.min.css' );
	wp_enqueue_style('wrgf-pricing-table-css', WEBLIZAR_SLGF_PLUGIN_URL.'css/pricing-table.css');
	wp_enqueue_style( 'wrgf-boot-strap-min', WEBLIZAR_SLGF_PLUGIN_URL . 'css/bootstrap-latest/bootstrap.min.css' );
...
function slgf_js_css_load_function() {
	...
	foreach ( $Posts as $Post ) {
		if ( isset( $Post->post_content ) && strpos( $Post->post_content, 'SLGF' ) ) {
			...
			wp_enqueue_style( 'wl-slgf-boot-strap-css', WEBLIZAR_SLGF_PLUGIN_URL . 'css/bootstrap-latest/bootstrap.css' );
...
class SLGF {
	...
	public function slgf_admin_print_scripts() {
		...
		//font awesome css
		wp_enqueue_style( 'slgf-font-awesome-5', WEBLIZAR_SLGF_PLUGIN_URL . 'css/font-awesome-latest/css/fontawesome-all.min.css' );
	...
	public function add_all_slgf_meta_boxes() {
		add_meta_box( __( 'Add Images', WEBLIZAR_SLGF_TEXT_DOMAIN ), __( 'Add Images', WEBLIZAR_SLGF_TEXT_DOMAIN ), array(
			&$this,
			'slgf_generate_add_image_meta_box_function'
		), 'slgf_slider', 'normal', 'low' );
		add_meta_box( __( 'Apply Setting On Lightbox Gallery', WEBLIZAR_SLGF_TEXT_DOMAIN ), __( 'Apply Setting On Lightbox Gallery', WEBLIZAR_SLGF_TEXT_DOMAIN ), array(
			&$this,
			'slgf_settings_meta_box_function'
		), 'slgf_slider', 'normal', 'low' );
		add_meta_box( __( 'Lightbox Gallery Shortcode', WEBLIZAR_SLGF_TEXT_DOMAIN ), __( 'Lightbox Gallery Shortcode', WEBLIZAR_SLGF_TEXT_DOMAIN ), array(
			&$this,
			'slgf_shotcode_meta_box_function'
		), 'slgf_slider', 'side', 'low' );
		add_meta_box( __( 'Lightbox Slider Pro', WEBLIZAR_SLGF_TEXT_DOMAIN ), __( 'Lightbox Slider Pro', WEBLIZAR_SLGF_TEXT_DOMAIN ), array(
			&$this,
			'slgf_upgrade_to_pro_function'
		), 'slgf_slider', 'side', 'low' );
		add_meta_box( __( 'Rate us on WordPress', WEBLIZAR_SLGF_TEXT_DOMAIN ), __( 'Rate us on WordPress', WEBLIZAR_SLGF_TEXT_DOMAIN ), array(
			&$this,
	...
	function slgf_rate_us_function() { ?>
        <div style="text-align:center">
            <h3><?php _e( "If you like our plugin then please show us some love", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?></h3>
            ...
            <div class="upgrade-to-pro-demo" style="text-align:center;margin-bottom:10px;margin-top:10px;">
                <a href="https://wordpress.org/plugins/simple-lightbox-gallery/#reviews" target="_new"
                   class="button button-primary button-hero"><?php _e( "Click Here", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?></a>
            </div>
        </div>
		<?php
	}
	...
	function slgf_upgrade_to_pro_function() {
        ...
        <div class="">
            <div class="update_pro_button"><a target="_blank"
                                              href="https://weblizar.com/lightbox-slider-pro/"><?php _e( 'Buy Now $12', WEBLIZAR_SLGF_TEXT_DOMAIN ); ?></a>
            </div>
            <div class="update_pro_image">
                <img class="slgf_getpro" src="<?php echo $imgpath; ?>">
            </div>
	...
	public function slgf_generate_add_image_meta_box_function( $post ) { ?>
				...
				if ( $TotalImages ) {
					foreach ( $SLGF_AllPhotosDetails as $SLGF_SinglePhotoDetails ) {
                        ...
                        <li class="rpg-image-entry" id="rpg_img">
                            ...
                            <label><?php _e( 'Description', WEBLIZAR_SLGF_TEXT_DOMAIN ); ?></label>
							<textarea name="img_desc[]" id="img_desc[]"
								  placeholder="<?php _e( 'Description', WEBLIZAR_SLGF_TEXT_DOMAIN ); ?>"
								  class="gal_richeditbox_<?php echo $i; ?>"><?php echo htmlentities( $img_desc ); ?>
						    </textarea>
                        </li>
						<?php
        ...
        <div class="rpg-image-entry add_rpg_new_image" id="slgf_gallery_upload_button"
            ...
            <p>
				<?php _e( 'Add New Images', WEBLIZAR_SLGF_TEXT_DOMAIN ); ?>
            </p>
        </div>
        ...
        <div class="rpg-image-entry del_rpg_image" id="slgf_delete_all_button">
            ...
            <p>
				<?php _e( 'Delete All', WEBLIZAR_SLGF_TEXT_DOMAIN ); ?>
            </p>
        </div>
        ...
        <p><strong><?php _e( "Tips", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?>
            :</strong> <?php _e( "Plugin crop images with same size thumbnails", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?>
        . <?php _e( "So please upload all gallery images using Add New Image button", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?>
        . <?php _e( "Do not use or add pre uploaded images which are uploaded previously using Media or Post or Page", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?>
        .</p><?php _e( "Show Us Some Love", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?>
        (<?php _e( "Rate Us", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?>) &nbsp;
...
function add_slgf_inline_popup_content() { ?>
    ...
    <div id="SLGF" style="display:none;">
        <h3><?php _e( "Select Lightbox Gallery To Insert Into Post", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?></h3>
		<?php
		...
		if ( $rpg_galleries->have_posts() ) { ?>
            ...
            <button class='button primary'
                    id='slgfgalleryinsert'><?php _e( "Insert Gallery Shortcode", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?></button>
			<?php
		} else {
...
function as_settings_link_slgf( $links ) {
	$as_settings_link1 = '<a href="https://weblizar.com/lightbox-slider-pro/" style="font-weight:700; color:#e35400" target="_blank">Go Pro</a>';
